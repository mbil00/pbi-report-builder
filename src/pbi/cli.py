...
def diff_cmd(
    ...
    if page is None and isinstance(spec.get("theme"), dict):
        ...
        current_theme = export_theme_spec(proj)
        plan = plan_theme_spec(proj, spec.get("theme", {}))
        if plan is not None:
            current_flat = flatten_diff_spec(current_theme)
            planned_flat = flatten_diff_spec(plan.payload)
            theme_diffs = []
            ...
            for key in sorted(keys):
                ...
                if curr != planned:
                    theme_diffs.append((key, curr or "(none)", planned or "(none)"))
            if theme_diffs:
                has_diffs = True
                console.print("\n[bold]Theme[/bold]")
