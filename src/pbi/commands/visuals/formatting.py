...
def visual_format_set(
    ...
    if mode == "rules":
        if not rule:
            ...
            raise typer.Exit(1)
        try:
            rules = [parse_conditional_formatting_rule(r) for r in rule]
    ...
    if mode == "measure":
        console.print(f"Set [cyan]{prop}[/cyan] = measure [bold]{source_label}[/bold]")
    elif mode == "rules":
        console.print(f"Set [cyan]{prop}[/cyan] = rules by [bold]{source_label}[/bold] ({len(rules)} rules)")
    else:
        stops = f"{min_stop.color}@{min_stop.value}" if min_stop and max_stop else ""
        if mid_stop:
            stops += f" -> {mid_stop.color}@{mid_stop.value}"
        if max_stop:
            stops += f" -> {max_stop.color}@{max_stop.value}"
        console.print(f"Set [cyan]{prop}[/cyan] = gradient by [bold]{source_label}[/bold] [{stops}]")
