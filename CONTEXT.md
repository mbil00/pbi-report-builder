...
**Apply Plan**:
The pure-function output of computing what one section of a **YAML Round-Trip** apply *would* write to a PBIR-substrate document, without performing the write. Each section has its own planner: the theme planner (`plan_theme_spec`) reads current theme JSON, merges the spec with `None`-means-delete semantics, defaults the theme name, and detects no-op; the report planner (`plan_report_spec`) reads `report.json`, normalizes `resourcePackages`, merges per top-level key with the same `None`-means-delete semantics, and detects no-op; the bookmarks planner (`plan_bookmarks_spec`) reads existing bookmark id/file-path pairs by display name, normalizes per-bookmark state through `normalize_bookmark_state`, and returns a flat list of per-bookmark write operations plus a `(display_name, group | None)` list for the final group reconcile. The plan carries the merged payload (or per-op payloads, for bookmarks), a `keys_touched` count for `ApplyResult` accounting, and any flags the persistence step needs (e.g. `first_time` for the theme write path). The apply engine drives the **Apply Session** to persist a plan; for the bookmarks plan, the engine issues one `session.write_bookmark` per operation and exactly one `session.reconcile_bookmark_groups` at the end, so the bookmarks meta file is written exactly once per apply. The same plan can be inspected by `pbi diff` without touching disk, so partial-key specs render the same diff that apply would actually write.
...
- An **Apply Plan** is computed for each PBIR-substrate document (theme, report, bookmarks) that a **YAML Round-Trip** apply would touch, and is persisted by the **Apply Session**. The bookmarks plan is a list of per-bookmark write operations plus one final group-reconcile call, so the bookmarks meta file is written exactly once per apply.
