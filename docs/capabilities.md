...
| Report-level metadata | `report get/set/properties` for schema-backed scalar settings, `report annotation list/get/set/delete`, `report object list|get|set|clear` for top-level objects, `report resource package ...` and `report resource item ...` for first-class `resourcePackages` editing, plus `report custom-visual list|get|set|delete` for `organizationCustomVisuals` | `dataSourceVariables` still lacks a dedicated helper |
