...
| Report-level metadata | `report get/set/properties` for schema-backed scalar settings, `report annotation list/get/set/delete`, `report object list/get/set/clear` for top-level objects like `resourcePackages`, `filterConfig`, `objects`, and `themeCollection` | no dedicated resource-package subgroup yet |
