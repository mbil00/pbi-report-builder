...
| Report-level metadata | `report get/set/properties` for schema-backed scalar settings, `report annotation list/get/set/delete`, `report object list/get/set/clear` for top-level objects, plus `report resource package ...` and `report resource item ...` for first-class `resourcePackages` editing | no dedicated `organizationCustomVisuals` subgroup yet |
