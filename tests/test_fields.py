...
from pbi.fields import resolve_binding_ref, resolve_field_info, resolve_field_type
...
def test_resolve_binding_ref_from_root_preserves_measure_marker() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        project = _make_project(Path(tmp), with_model=False)

        assert resolve_binding_ref(project.root, "Metrics.Total Revenue (measure)") == (
            "Metrics",
            "Total Revenue",
            "measure",
        )
