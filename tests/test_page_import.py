...
def test_import_page_rewrites_visual_ids_and_group_references() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        first = ReportAuthoring(source).create_visual(source_page, "cardVisual", x=0, y=0, width=100, height=50)
        second = ReportAuthoring(source).create_visual(source_page, "cardVisual", x=120, y=0, width=100, height=50)
        group = ReportAuthoring(source).create_group(source_page, [first, second], display_name="Cards")
...
def test_import_page_copies_registered_resources_when_requested() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        source_page = ReportAuthoring(source).create_page("Source")
        visual = ReportAuthoring(source).create_visual(source_page, "image", x=0, y=0, width=100, height=50)
